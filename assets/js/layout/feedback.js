...
function initFeedBack() {
    //獲取事件並以時間降冪排序
    const messages = axios.get(`${_url}/messages?_sort=reviewDateTime&_order=desc`)
    const ratings = axios.get(`${_url}/ratings?_sort=reviewDateTime&_order=desc`)

    Promise.all([messages, ratings])
        .then(function (res) {
            ratingsDatas = res[1].data
            messagesDatas = res[0].data
        ...
        <div
                            ...
                            <div class="d-flex mb-12 mb-md-16 w-100">
                                <img class="evaluate-store w-100 d-block border border-2 border-line position-absolute"
                                    src="${data.reviewerPhoto}" alt="store-logo">
                            ...
                            <p class="px-12 px-md-24 lh-lg doubleline-ellipsis">${data.feedbackText}
                            </p>
                        </div>
                        `
    })
        ...
        <div class="d-flex mb-12 mb-md-16">
            ...
            <div
                ...
                <time datetime="2023-08-06"
                    class="user-date fs-16 d-block ms-auto ">${data.reviewDateTime
            }</time>
            </div>
        </div>
