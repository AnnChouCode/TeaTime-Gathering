import{b as y}from"./main-683b5b67.js";import{i as b,a as p}from"./nav-6adc6ebb.js";import{i as x}from"./isPastEvent-33573b9d.js";const g="https://teatimeapi-test.onrender.com",f="U004";let D=localStorage.getItem("token");const V=moment().format("YYYY/MM/DD");let c=[];const o={};function S(){const e=p.get(`${g}/groupings?_expand=restaurant`),s=p.get(`${g}/votings?_expand=restaurant`);Promise.all([e,s]).then(function(n){c=c.concat(n[0].data),c=c.concat(n[1].data),E()}).catch(function(n){console.error(n)})}S();function E(){c.forEach(e=>{o[e.UID]||(o[e.UID]={UID:e.UID,restaurantId:e.restaurantId,deadlineDateTime:e.deadlineDateTime,eventDateTime:e.eventDateTime,initiator:e.initiator,invitees:e.invitees,restaurantList:e.UID.startsWith("V")?[]:[e.restaurantName],restaurantPhoto:e.restaurant.storeCover,minGroupSize:e.restaurant.minGroupSize,currentGroupCondition:e.currentGroupCondition}),e.UID.startsWith("V")&&(o[e.UID].restaurantList.push(e.restaurantName),delete o[e.UID].currentGroupCondition,delete o[e.UID].minGroupSize)}),w(),I()}function w(){const e=[];for(const s in o)if(o.hasOwnProperty(s)){const n=o[s];e.push({id:n.UID,name:n.restaurantList.join("、"),date:n.deadlineDateTime,type:"event",color:n.UID[0]==="V"?"#F0BD68":"#FD909F"})}U(e)}function U(e){$("#calendar").evoCalendar({sidebarDisplayDefault:!1,sidebarToggler:!0,eventDisplayDefault:!1,eventListToggler:!1,todayHighlight:!0,calendarEvents:e})}$("#calendar").on("click",function(e){const s=$("#calendar").evoCalendar("getActiveDate"),n=moment(s,"MM/DD/YYYY").format("YYYY/MM/DD");I(n)});const C=document.querySelector(".calendarEvent");function I(e=V){let s="";const a=Object.values(o).filter(t=>t.deadlineDateTime.split(" ")[0]===e);a.length?a.forEach(t=>{s+=`<li ${t.UID.startsWith("V")?`class="cursor-pointer" data-bs-votingUID=${t.UID} data-bs-toggle="modal" data-bs-target="#modal-VotingModal"`:`data-groupingUID=${t.UID}`}>
                ...
                ${t.UID.startsWith("V")?"":"</a>"}
              </li>`}):s+='<li class="fs-16 fs-md-20 text-center">目前沒有下午茶活動</li>',C.innerHTML=s}function L(e,s){let n;return x(e)?n=`<div class="align-self-end py-4 py-md-8 order-last order-md-first rounded-pill text-center btn-disable" style="width:110px;">${s.startsWith("V")?"投票":"開團"}已結束</div>`:n=`<div class="align-self-end py-4 py-md-8 order-last order-md-first ${s.startsWith("V")?"bg-brand-01":"bg-brand-02"} rounded-pill text-white text-center" style="width:80px">${s.startsWith("V")?"投票中":"開團中"}</div>`,n}function m(e,s){return Math.floor(e/s*100)}const l=document.getElementById("modal-VotingModal");function T(e){let s=[];p.get(`${g}/votings?UID=${e}&_expand=restaurant`).then(function(n){s=n.data,Y(s)}).catch(function(n){console.error(n)})}l.addEventListener("show.bs.modal",function(e){const n=e.relatedTarget.getAttribute("data-bs-votingUID");M(n),T(n),b(D)});function M(e){const s=l.querySelector(".modal-title");s.textContent=o[e].restaurantList.join("、");const n=l.querySelector(".deadlineDateTime"),a=l.querySelector(".eventDateTime"),t=l.querySelector(".initiator"),i=l.querySelector(".invitees"),r=o[e];n.innerHTML=`<span class="me-16">截止日期</span>
                      ...
                      </li>`}),s.innerHTML=a,q()}function G(e,s,n){return s?'<button class="py-4 px-16 btn-disable border-0 rounded-pill btnVoteThis" disabled type="button">已結束</button>':b(D)?`<button
