import{b as h}from"./main-f70f762a.js";import{i as f}from"./notification-2a8e3c71.js";import{a as u}from"./axios-3a76d256.js";import{i as y}from"./isPastEvent-33573b9d.js";const p="https://teatimeapi-test.onrender.com",v="U004",b=localStorage.getItem("token"),V=moment().format("YYYY/MM/DD");let c=[];const r={};function S(){const e=u.get(`${p}/groupings?_expand=restaurant&_expand=order`),s=u.get(`${p}/votings?_expand=restaurant`);Promise.all([e,s]).then(function(n){c=c.concat(n[0].data),c=c.concat(n[1].data),E()}).catch(function(n){console.error(n.message)})}S();function E(){c.forEach(e=>{r[e.UID]||(r[e.UID]={UID:e.UID,restaurantId:e.restaurantId,deadlineDateTime:e.deadlineDateTime,eventDateTime:e.eventDateTime,initiator:e.initiator,invitees:e.invitees,restaurantList:e.UID.startsWith("V")?[]:[e.restaurant.storeName],restaurantPhoto:e.restaurant.storeCover,minGroupSize:e.restaurant.minGroupSize,currentGroupCondition:e.UID.startsWith("V")?0:e.order.orderDetail.length}),e.UID.startsWith("V")&&(r[e.UID].restaurantList.push(e.restaurantName),delete r[e.UID].currentGroupCondition,delete r[e.UID].minGroupSize)}),w(),x()}document.addEventListener("DOMContentLoaded",function(){const e=document.querySelector(".btnCreateVoting");f(b)?(e.setAttribute("href","#modal-CreateVoting"),e.setAttribute("data-bs-toggle","modal")):(e.setAttribute("data-bs-toggle","popover"),e.setAttribute("data-bs-content","請先登入帳號"),new h.Popover(e))});function w(){const e=[];for(const s in r)if(r.hasOwnProperty(s)){const n=r[s];e.push({id:n.UID,name:n.restaurantList.join("、"),date:n.deadlineDateTime,type:"event",color:n.UID[0]==="V"?"#F0BD68":"#FD909F"})}U(e)}function U(e){$("#calendar").evoCalendar({sidebarDisplayDefault:!1,sidebarToggler:!0,eventDisplayDefault:!1,eventListToggler:!1,todayHighlight:!0,calendarEvents:e})}$("#calendar").on("click",function(e){const s=$("#calendar").evoCalendar("getActiveDate"),n=moment(s,"MM/DD/YYYY").format("YYYY/MM/DD");x(n)});const L=document.querySelector(".calendarEvent");function x(e=V){let s="";const o=Object.values(r).filter(t=>t.deadlineDateTime.split(" ")[0]===e);o.length?o.forEach(t=>{s+=`<li ${t.UID.startsWith("V")?`class="cursor-pointer" data-bs-votingUID=${t.UID} data-bs-toggle="modal" data-bs-target="#modal-VotingModal"`:`data-groupingUID=${t.UID}`}>
                ...
                ${t.UID.startsWith("V")?"":"</a>"}
              </li>`}):s+='<li class="fs-16 fs-md-20 text-center">目前沒有下午茶活動</li>',L.innerHTML=s}function C(e,s){let n;return y(e)?n=`<div class="align-self-end py-4 py-md-8 order-last order-md-first rounded-pill text-center btn-disable" style="width:110px;">${s.startsWith("V")?"投票":"開團"}已結束</div>`:n=`<div class="align-self-end py-4 py-md-8 order-last order-md-first ${s.startsWith("V")?"bg-brand-01":"bg-brand-02"} rounded-pill text-white text-center" style="width:80px">${s.startsWith("V")?"投票中":"開團中"}</div>`,n}function m(e,s){return Math.floor(e/s*100)}const l=document.getElementById("modal-VotingModal");function I(e){let s=[];u.get(`${p}/votings?UID=${e}&_expand=restaurant`).then(function(n){s=n.data,Y(s)}).catch(function(n){console.error(n.message)})}l.addEventListener("show.bs.modal",function(e){const n=e.relatedTarget.getAttribute("data-bs-votingUID");M(n),I(n)});function M(e){const s=l.querySelector(".modal-title");s.textContent=r[e].restaurantList.join("、");const n=l.querySelector(".deadlineDateTime"),o=l.querySelector(".eventDateTime"),t=l.querySelector(".initiator"),i=l.querySelector(".invitees"),a=r[e];n.innerHTML=`<span class="me-16">截止日期</span>
                        ...
                        <div class="flex-grow-1 d-flex flex-column">
                          ...
                          <div class="d-flex justify-content-between flex-column flex-lg-row">
                            <div class="d-flex align-items-center mb-8 mb-lg-0 ">
                      ...
                      </li>`}),s.innerHTML=o,A()}function q(e,s,n){return s?'<button class="py-4 px-16 btn-disable border-0 rounded-pill btnVoteThis" disabled type="button">已結束</button>':f(b)?`<button
                              ...
                              data-bs-container="body" data-bs-toggle="popover"  data-bs-placement="top" data-bs-content="請先登入帳號">投票</button>`}function A(){if(!f(b)){[...document.querySelectorAll('[data-bs-toggle="popover"]')].map(n=>{new h.Popover(n)});return}l.querySelectorAll(".btnVoteThis").forEach(s=>{s.addEventListener("click",function(){s.classList.contains("btn-active-brand-02")?D(s.dataset.num,"deleteVote"):D(s.dataset.num,"addVote")})})}async function D(e,s){try{const n=document.querySelector(`.ts-progress[data-num="${e}"]`);n.classList.add("is-processing");const o=c.filter(t=>t.UID.startsWith("V")&&t.id==e)[0].currentVoters;if(s==="addVote")o.push(`${v}`);else{const t=o.indexOf(`${v}`);o.splice(t,1)}u.patch(`${p}/votings/${e}`,{currentVoters:o}).then(function(t){I(t.data.UID),n.classList.remove("is-processing")}).catch(function(t){console.error(t.message)})}catch(n){throw console.log("錯誤:",n),n}}const T=document.getElementById("modal-CreateVoting");T.addEventListener("show.bs.modal",function(e){const s=document.getElementById("votingInitiator");u.get(`${p}/users?UID=${v}`).then(function(n){s.textContent=n.data[0].userName}).catch(function(n){console.error(n.message)})});T.addEventListener("click",()=>{const e=document.getElementById("eventDateTime"),s=document.getElementById("deadlineDateTime"),n=document.getElementById("votingInvitees"),o=e.value.trim(),t=s.value.trim();let i="",a="";o&&(i=moment(e.value,"YYYY/MM/DD HH:mm")),t&&(a=moment(s.value,"YYYY/MM/DD HH:mm")),i&&a&&(i.diff(a,"hours")<24?document.querySelector(".deadlineDateTimeAlert").classList.remove("d-none"):document.querySelector(".deadlineDateTimeAlert").classList.add("d-none")),sessionStorage.setItem("deadlineDateTime",t),sessionStorage.setItem("eventDateTime",o),sessionStorage.setItem("votingInvitees",n.value)});const G=document.querySelector(".btnLinkToCreateVoting");G.addEventListener("click",function(){window.location.href="./createVoting.html"});
