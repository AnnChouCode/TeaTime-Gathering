import{b as h}from"./main-683b5b67.js";import{i as f,a as u}from"./nav-a283da62.js";import{i as y}from"./isPastEvent-33573b9d.js";const p="https://teatimeapi-test.onrender.com",v="U004",b=localStorage.getItem("token"),V=moment().format("YYYY/MM/DD");let c=[];const r={};function S(){const e=u.get(`${p}/groupings?_expand=restaurant`),s=u.get(`${p}/votings?_expand=restaurant`);Promise.all([e,s]).then(function(n){c=c.concat(n[0].data),c=c.concat(n[1].data),E()}).catch(function(n){console.error(n)})}S();function E(){c.forEach(e=>{r[e.UID]||(r[e.UID]={UID:e.UID,restaurantId:e.restaurantId,deadlineDateTime:e.deadlineDateTime,eventDateTime:e.eventDateTime,initiator:e.initiator,invitees:e.invitees,restaurantList:e.UID.startsWith("V")?[]:[e.restaurantName],restaurantPhoto:e.restaurant.storeCover,minGroupSize:e.restaurant.minGroupSize,currentGroupCondition:e.currentGroupCondition}),e.UID.startsWith("V")&&(r[e.UID].restaurantList.push(e.restaurantName),delete r[e.UID].currentGroupCondition,delete r[e.UID].minGroupSize)}),w(),x()}function w(){const e=[];for(const s in r)if(r.hasOwnProperty(s)){const n=r[s];e.push({id:n.UID,name:n.restaurantList.join("、"),date:n.deadlineDateTime,type:"event",color:n.UID[0]==="V"?"#F0BD68":"#FD909F"})}L(e)}function L(e){$("#calendar").evoCalendar({sidebarDisplayDefault:!1,sidebarToggler:!0,eventDisplayDefault:!1,eventListToggler:!1,todayHighlight:!0,calendarEvents:e})}$("#calendar").on("click",function(e){const s=$("#calendar").evoCalendar("getActiveDate"),n=moment(s,"MM/DD/YYYY").format("YYYY/MM/DD");x(n)});const U=document.querySelector(".calendarEvent");function x(e=V){let s="";const o=Object.values(r).filter(t=>t.deadlineDateTime.split(" ")[0]===e);o.length?o.forEach(t=>{s+=`<li ${t.UID.startsWith("V")?`class="cursor-pointer" data-bs-votingUID=${t.UID} data-bs-toggle="modal" data-bs-target="#modal-VotingModal"`:`data-groupingUID=${t.UID}`}>
                ...
                ${t.UID.startsWith("V")?"":"</a>"}
              </li>`}):s+='<li class="fs-16 fs-md-20 text-center">目前沒有下午茶活動</li>',U.innerHTML=s}function C(e,s){let n;return y(e)?n=`<div class="align-self-end py-4 py-md-8 order-last order-md-first rounded-pill text-center btn-disable" style="width:110px;">${s.startsWith("V")?"投票":"開團"}已結束</div>`:n=`<div class="align-self-end py-4 py-md-8 order-last order-md-first ${s.startsWith("V")?"bg-brand-01":"bg-brand-02"} rounded-pill text-white text-center" style="width:80px">${s.startsWith("V")?"投票中":"開團中"}</div>`,n}function m(e,s){return Math.floor(e/s*100)}const l=document.getElementById("modal-VotingModal");function T(e){let s=[];u.get(`${p}/votings?UID=${e}&_expand=restaurant`).then(function(n){s=n.data,Y(s)}).catch(function(n){console.error(n)})}l.addEventListener("show.bs.modal",function(e){const n=e.relatedTarget.getAttribute("data-bs-votingUID");M(n),T(n)});function M(e){const s=l.querySelector(".modal-title");s.textContent=r[e].restaurantList.join("、");const n=l.querySelector(".deadlineDateTime"),o=l.querySelector(".eventDateTime"),t=l.querySelector(".initiator"),i=l.querySelector(".invitees"),a=r[e];n.innerHTML=`<span class="me-16">截止日期</span>
                      ...
                      </li>`}),s.innerHTML=o,q()}function G(e,s,n){return s?'<button class="py-4 px-16 btn-disable border-0 rounded-pill btnVoteThis" disabled type="button">已結束</button>':f(b)?`<button
