import{b as K}from"./main-91b5cbf3.js";import{c as Q,i as H}from"./notification-93dbf0dc.js";import{s as f}from"./showStars-385adb56.js";import{a as d}from"./enc-utf8-05a2ad8c.js";import{i as Y}from"./isPastEvent-33573b9d.js";import{s as g}from"./showDateTime-51bda516.js";let E=[];const k=new Date;let P=`${k.getFullYear()}/${k.getMonth()+1}/${k.getDate()}`;A();function A(e){let t=`https://teatimeapi-test.onrender.com/groupings?deadlineDateTime_like=${P}&isGroup=true`;e==1&&(t="https://teatimeapi-test.onrender.com/groupings?&isGroup=true&_limit=4"),d.get(t).then(function(s){const r=s.data;if(r=="")A(1);else if(r.forEach((n,a)=>{const[i,o]=n.deadlineDateTime.split(" ");n.datePart=i,n.timePart=o,i==P?n.todayGroupings=!0:n.todayGroupings=!1,E.push(n)}),E!=""){let n="";const a=document.querySelector(".bannerName");X(E).then(i=>{n+=`
      ...
      `,e.length==2?t+=`
          ...
          `)),t+="</div>",t}const y="https://teatimeapi-test.onrender.com",V=localStorage.getItem("token"),L=Q(V);let u=[];function R(){const e=moment().add(2,"month").format("YYYY/MM"),s=`deadlineDateTime_gte=${moment().subtract(1,"month").format("YYYY/MM")}&deadlineDateTime_lte=${e}`,r=d.get(`${y}/groupings?_expand=restaurant&${s}`),n=d.get(`${y}/votings?_expand=restaurant&${s}`);Promise.all([r,n]).then(function(a){u=u.concat(a[0].data),u=u.concat(a[1].data),ee()}).catch(function(a){console.error(a.message)})}R();const b={};function ee(){u.sort((e,t)=>{const s=new Date(e.deadlineDateTime),r=new Date(t.deadlineDateTime);return s-r}),u.forEach(e=>{b[e.UID]||(b[e.UID]={UID:e.UID,restaurantId:e.restaurantId,deadlineDateTime:e.deadlineDateTime,eventDateTime:e.eventDateTime,initiator:e.initiator,invitees:e.invitees,restaurantList:e.UID.startsWith("V")?[]:[e.restaurant.storeName],restaurantPhoto:e.restaurant.storeCover}),e.UID.startsWith("V")&&b[e.UID].restaurantList.push(e.restaurantName)}),W()}const te=document.querySelectorAll(".btnfilterEvent"),j=document.querySelector(".calendarBlock");te.forEach(e=>{e.addEventListener("click",function(){const t=this.getAttribute("data-event");W(t)})});function W(e="groupings"){const t=Object.values(b).filter(n=>n.UID.startsWith(e[0].toUpperCase())),s=t.filter(n=>{const a=moment(n.deadlineDateTime,"YYYY/MM/DD HH:mm");return moment().isBefore(a)});let r;s.length>=4?r=s.slice(0,4):r=t.slice(t.length-4,t.length),se(r)}function se(e){if(!e.length){j.innerHTML="<div class='fs-16 fs-md-20 text-center'><p>目前沒有活動</p><p>歡迎建立開團與投票</p></div>";return}let t="";e.forEach(s=>{t+=`<div class="col-md-6 col-xl-4 col-xxxl-3">
                <a ${re(s.UID)}
                    ...
                    <div class="d-flex flex-column justify-content-between w-60 w-lg-100 flex-grow-1">
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <span class="fs-16 fs-lg-20 lh-sm fw-medium">${s.invitees?s.invitees:"無"}</span>
            ...
            </div>`}),j.innerHTML=t}function ne(e){let t;return Y(e)?t="<div class='fw-medium lh-sm fs-16 fs-lg-20'>已結束</div>":t=`<div class="d-flex align-items-center ms-8 gap-md-8 gap-4">
                                ...
                                </div>`,t}function re(e){return e.startsWith("V")?`href="#modal-VotingModal" data-bs-toggle="modal" data-bs-votingUID="${e}"`:`href="store-order.html?UID=${e}"`}const m=document.getElementById("modal-VotingModal");function F(e){let t=[];d.get(`${y}/votings?UID=${e}&_expand=restaurant`).then(function(s){t=s.data,ae(t)}).catch(function(s){console.error(s.message)})}m.addEventListener("show.bs.modal",function(e){const s=e.relatedTarget.getAttribute("data-bs-votingUID");ie(s),F(s)});function ie(e){const t=m.querySelector(".modal-title");t.textContent=b[e].restaurantList.join("、");const s=m.querySelector(".deadlineDateTime"),r=m.querySelector(".eventDateTime"),n=m.querySelector(".initiator"),a=m.querySelector(".invitees"),i=b[e];s.innerHTML=`<span class="me-16">截止日期</span>
    ...
    <span>${g(i.eventDateTime)[1]}</span>`,n.textContent=i.initiator,a.textContent=i.invitees===""?"無":i.invitees}function ae(e){const t=m.querySelector(".votingCard"),s=Y(e[0].deadlineDateTime);let r="";e.forEach(n=>{const a=n.currentVoters.includes(L),i=n.currentVoters.length,o=n.restaurant.minGroupSize;r+=`
    <li class="d-flex gap-12 p-12 mb-12 border border-brand-03 border-radius-40401616">
        <img class="border-radius-32321616 votingcard-photo" src="${n.restaurant.storeCover}">
            ...
            </li>`}),t.innerHTML=r,le()}function I(e,t){return Math.floor(e/t*100)}function oe(e,t,s){return t?'<button class="py-4 px-16 btn-disable border-0 rounded-pill btnVoteThis" disabled type="button">已結束</button>':H(V)?`<button
            ...
            type="button" data-num=${e}>${s?"已投票":"投票"}</button>`:`<button
                      ...
                      type="button" data-num=${e} data-bs-container="body" data-bs-toggle="popover" data-bs-placement="top" data-bs-content="請先登入帳號">投票</button>`}function le(){H(V)||[...document.querySelectorAll('[data-bs-toggle="popover"]')].map(s=>{new K.Popover(s)}),m.querySelectorAll(".btnVoteThis").forEach(t=>{t.addEventListener("click",function(){this.disabled=!0,setTimeout(()=>{this.disabled=!1},3e3),t.classList.contains("btn-active-brand-02")?C(t.dataset.num,"deleteVote"):C(t.dataset.num,"addVote")})})}async function C(e,t){try{const s=document.querySelector(`.ts-progress[data-num="${e}"]`);s.classList.add("is-processing");const r=u.filter(n=>n.UID.startsWith("V")&&n.id==e)[0].currentVoters;if(t==="addVote")r.push(`${L}`);else{const n=r.indexOf(`${L}`);r.splice(n,1)}d.patch(`${y}/votings/${e}`,{currentVoters:r}).then(function(n){F(n.data.UID),s.classList.remove("is-processing")}).catch(function(n){console.error(n.message)})}catch(s){throw console.log("錯誤:",s),s}}$(".btnfilterEvent").click(function(e){$(".btnfilterEvent").toggleClass("btn-active-brand-02")});let _=document.querySelectorAll(".select-bar-link");_.forEach(e=>{e.addEventListener("click",()=>{_.forEach(t=>{t.classList.remove("btn-active-brand-02")}),e.classList.add("btn-active-brand-02")})});const G="https://teatimeapi-test.onrender.com";moment().format("YYYY/MM/DD");let p={category:"全部",sort:"",search:""};const ce={newestStore:"&_sort=id&_order=desc",bestStore:"&_sort=stars&_order=desc",worstStore:"&_sort=stars&_order=asc"};function O(e,t,s){let r="",n="",a="";e!=="全部"&&e&&(r=`category=${e}`),t&&(n=t),s&&(a=`&storeName_like=${s}`);let i=`${G}/restaurants?${r}${n}${a}`;me(i)}O();const de=document.querySelectorAll(".btnSortStore");de.forEach(e=>{e.addEventListener("click",function(){const t=this.getAttribute("data-sort-value");x("sort",t)})});const B=document.querySelectorAll(".btnCategoryStore");B.forEach(e=>{e.addEventListener("click",function(){const t=this.getAttribute("data-category-value");B.forEach(s=>{s.classList.remove("actived")}),x("category",t),e.classList.add("actived")})});function x(e,t){switch(e){case"category":p[e]=t;break;case"search":p[e]=t.trim();break;default:p[e]=ce[t];break}O(p.category,p.sort,p.search)}function me(e){$(function(){(function(t){let s=$("#pagination"+t);$.ajax({url:e,success:function(r){ue(r,s)},error:function(r){console.error(r)}})})("Pages")})}function ue(e,t){t.pagination({dataSource:e,totalNumber:e.length,pageSize:8,showPageNumbers:!0,showPrevious:!0,showNext:!0,callback:function(s){pe(s)}})}function pe(e){const t={飲料:"icon-park-outline:drink",速食:"mdi:food-outline",素食:"tabler:leaf",小吃:"ph:bowl-food",甜點:"material-symbols:icecream-outline-rounded",咖啡:"icon-park-outline:tea-drink"};let s='<ul class="row row-cols-2 gap-12 gap-xxl-24 findStore-ul">';e.forEach((r,n)=>{s+=`
        ...
        `}),s+="</ul>",document.querySelector("#paginationContainer").innerHTML=s}const U=new autoComplete({data:{src:async e=>{try{return await(await fetch(`${G}/restaurants`)).json()}catch(t){return t}},keys:["storeName"]},searchEngine:"loose",debounce:300,resultItem:{highlight:!0},resultsList:{element:(e,t)=>{if(!t.results.length){const s=document.createElement("div");s.setAttribute("class","no_result px-8"),s.innerHTML=`<span>Found No Results for "${t.query}"</span>`,e.prepend(s)}},noResults:!0},events:{input:{selection:e=>{const t=e.detail.selection.value;U.input.value=t.storeName,x("search",t.storeName)},keydown:e=>{if(e.keyCode===13){const t=U.input.value;x("search",t)}}}}}),fe=document.getElementById("autoComplete");fe.addEventListener("input",function(e){x("search","")});const q="https://teatimeapi-test.onrender.com";let D="",J=[],z=[];function ge(){const e=d.get(`${q}/messages?_sort=reviewDateTime&_order=desc`),t=d.get(`${q}/ratings?_sort=reviewDateTime&_order=desc`);Promise.all([e,t]).then(function(s){z=s[1].data,J=s[0].data,M()}).catch(function(s){console.error(s.message)})}ge();function M(){window.addEventListener("resize",be);let e=JSON.parse(JSON.stringify(z)),t=JSON.parse(JSON.stringify(J));D||(e=e.slice(0,4),t=t.slice(0,4));let s="",r="";e.forEach(i=>{const o=f(i.starRating),v=o.replace(/width="16"/g,'width="24"');s+=`
        <div
    ...
    </li>`});const n=document.querySelector(".ratingsContainer");n.innerHTML=s;const a=document.querySelector(".messagesContainer");a.innerHTML=r}let N;function be(){clearTimeout(N),N=setTimeout(ve,500)}function ve(){window.innerWidth<992?(D=!0,M()):(D=!1,M())}
