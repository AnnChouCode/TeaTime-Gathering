import{b as L}from"./main-96991ec2.js";import{i as Ie,c as ue}from"./notification-39e0ad74.js";import{a as g}from"./enc-utf8-c5181946.js";import{s as le}from"./showDateTime-51bda516.js";import{s as pe}from"./showStars-385adb56.js";import{f as ge}from"./index-a4e39586.js";const De=localStorage.getItem("token");document.addEventListener("DOMContentLoaded",function(){const t=document.querySelector(".btnCreateGrouping"),s=document.querySelectorAll(".btnCart"),e=document.querySelectorAll(".btnWriteReview");Ie(De)?(t.setAttribute("href","#modal-CreateGroup"),t.setAttribute("data-bs-toggle","modal"),s.forEach(a=>{}),e.forEach(a=>{a.setAttribute("href","#modal-WriteReview"),a.setAttribute("data-bs-toggle","modal")})):(t.setAttribute("data-bs-toggle","popover"),t.setAttribute("data-bs-content","請先登入帳號"),s.forEach(n=>{n.setAttribute("data-bs-toggle","popover"),n.setAttribute("data-bs-content","請先登入帳號")}),e.forEach(n=>{n.setAttribute("data-bs-toggle","popover"),n.setAttribute("data-bs-content","請先登入帳號")}),[...document.querySelectorAll('[data-bs-toggle="popover"]')].map(n=>{new L.Popover(n)}))});const Te=moment().format("YYYY/MM/DD"),Se="https://teatimeapi-test.onrender.com",F=location.href.split("=")[1],ke=document.querySelector(".btnTogetherEvent");ke.addEventListener("click",Ee);let _;function Ee(){const t=`_sort=deadlineDateTime&deadlineDateTime_gte=${Te}`;g.get(`${Se}/groupings?_expand=restaurant&${t}`).then(function(s){_=s.data;let e,a;F.startsWith("G")?(e=_.filter(r=>r.UID===F),a=e.length?e[0].restaurantId:""):(e=_.filter(r=>r.restaurant.UID===F),a=e.length?e[0].restaurantId:"");const n=_.filter(r=>r.restaurantId===a);je(n)}).catch(function(s){console.error(s.message)})}function je(t){const s=document.querySelector(".calendarBlock");let e="";t.length?t.forEach(a=>{e+=`<div class="col-md-6 col-xl-4 col-xxxl-3 px-0 px-md-12">
                        <a href="store-order.html?UID=${a.UID}"
                            ...
                            <img class="event-img-RWD me-12 me-lg-0 mb-lg-20"
                                src="${a.restaurant.storeCover}"
                                alt="calendar-img">
                                <div class="d-flex flex-column justify-content-between w-60 w-lg-100 flex-grow-1">
                        <p class="mb-8 mb-lg-16 fs-20 fs-lg-24 fw-bold lh-sm text-brand-03 single-ellipsis">${a.restaurant.storeName}</p>
                        <div class="d-flex justify-content-between align-items-center mb-8 mb-lg-16">
                            ...
                            <div class="d-flex align-items-center ms-8 gap-md-8 gap-4">
                                    ...
                                    <time class="fw-medium lh-sm fs-16 fs-lg-20">${le(a.deadlineDateTime)[0]}</time>
                                    <time class="fw-medium lh-sm fs-16 fs-lg-20">${le(a.deadlineDateTime)[1]}</time>
                                </div>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <p class="fs-16 fs-lg-20 text-gray-05">請客人</p>
                            <span class="fs-16 fs-lg-20 lh-sm fw-medium">${a.invitees?a.invitees:"無"}</span>
                        </div>
                    </div>
                        </a>
                    </div>`}):e="<p class='fs-16 fs-md-20 text-center'>目前沒有開團</p>",s.innerHTML=e}function se(t="",s=""){let e={eventDateTime:{},deadlineDateTime:{}};if(t!=""){const[a,n]=t.split(" "),l=new Date(t).toLocaleDateString("en-US",{month:"numeric",day:"numeric"});e.eventDateTime.yymmdd=a,e.eventDateTime.date=l,e.eventDateTime.time=n}if(s!=""){const[a,n]=s.split(" "),l=new Date(s).toLocaleDateString("en-US",{month:"numeric",day:"numeric"});e.deadlineDateTime.yymmdd=a,e.deadlineDateTime.date=l,e.deadlineDateTime.time=n}return e}function fe(t=0,s=0){return Math.floor(t/s*100)}function Ae(t){const s=new Date().getTime(),e=Date.parse(t);return s>e}let Me=localStorage.getItem("token");const P=ue(Me);localStorage.setItem("Carts","");localStorage.setItem("category","");const ie=document.querySelector(".shopping-cart");document.querySelector(".shopping-cart-mobile ");const Be=document.getElementById("modalCartsSendOrder");$(function(){(function(){new L.Modal(document.getElementById("modal-shppingCart-order")),document.getElementById("staticBackdrop");let t=location.href.split("=")[1],s="";const e=!!t.startsWith("G");e?g.get(`https://teatimeapi-test.onrender.com/groupings?UID=${t}&_expand=restaurant&_expand=order`).then(a=>{if(!a.data[0]){$("#modal-storeOrder-eventExpired").modal("show"),$("#storeOrderClose").on("click",function(){window.location.href="https://annchoucode.github.io/TeaTime-Gathering"});return}const{restaurantId:n,deadlineDateTime:r}=a.data[0];Ae(r)&&new L.Popover(ie,{content:"揪團活動已截止"}),s=n,de(e,t,s,a.data[0]),Ue(a.data[0])}).catch(a=>{console.log(a)}):(new L.Popover(ie,{content:"目前您未選擇揪團活動，請先選擇揪團中或建立揪團"}),de(e,t,s,""))})()});function Ue(t){$(".shopping-cart").on("click",function(){if(P)$("#modal-shppingCart-order").modal("show");else return;localStorage.getItem("category");let s="";t.storeName=$("#storeNameID").text();const e=se(t.eventDateTime,t.deadlineDateTime);t.yymmdd=e.eventDateTime.yymmdd,t.date=e.eventDateTime.date,t.time=e.eventDateTime.time,$("#modalShppingCartOrderTitle").html(`<p class="fs-20 fs-md-24 line-height-sm fw-bold text-gray-01" id="modalShppingCartOrderTitle">${t.storeName}</p>`),$("#dateCarts").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="dateCarts">${e.eventDateTime.date}</p>`),$("#timeCarts").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="timeCarts">${e.eventDateTime.time}</p>`),$("#dateCartsOrder").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="dateCartsOrder">${e.deadlineDateTime.date}</p>`),$("#timeCartsOrder").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="timeCartsOrder">${e.deadlineDateTime.time}</p>`);const a=localStorage.getItem("Carts");if(a.length!==0){$(".modalCartsFooter").show();const n=JSON.parse(a);let r=0;n.forEach(o=>{r+=parseInt(o.totalAmount);let f="";o.customization.length!==0&&(f=o.customization.join(", ")),s+=`
      ...
      `,$("#menuListUl").html(s),$(".modalCartsFooter").hide()}),Be.addEventListener("click",function(){_e(t)})}function _e(t){if(!P)return;$("#modal-shppingCart-order").modal("hide"),$("#modal-successfullyOrdered").modal("show");const s=document.getElementById("menuTotal"),e=parseInt(s.textContent.replace("$","")),a=JSON.parse(localStorage.getItem("Carts")),n=JSON.parse(JSON.stringify(a));t.orderPriceTotal=e,t.orderUserId=P,g.get(`https://teatimeapi-test.onrender.com/users?UID=${P}`).then(r=>{const{userName:l}=r.data[0],{orderId:o,UID:f}=t,v=f;return t.userName=l,g.get(`https://teatimeapi-test.onrender.com/orders?id=${o}`).then(d=>{const u=d.data[0],{orderDetail:I,groupingId:j,restaurantId:D,UID:C}=u,w=[];let T="";return I.forEach((i,x)=>{a.forEach((c,A)=>{c.customization.length!=0&&(T=c.customization.join("、")),i.name==l&&i.orderItem==c.orderItem&&(w.push(A),I[x]={name:l,orderItem:c.orderItem,quantity:parseInt(c.quantity),ice:c.ice?`${c.ice}`:"",sugar:c.sugar?`${c.sugar}`:"",customization:c.customization?`${T}`:"",comments:c.comments,totalAmount:c.totalAmount,isPay:i.isPay,ratingID:i.ratingID})})}),w.length>0&&w.reverse().forEach(x=>{n.splice(x,1)}),n.forEach(i=>{let x="";i.customization.length!=0&&(x=i.customization.join("、"));const c={name:l,orderItem:i.orderItem,quantity:parseInt(i.quantity),ice:i.ice?`${i.ice}`:"",sugar:i.sugar?`${i.sugar}`:"",customization:i.customization?`${x}`:"",comments:i.comments,totalAmount:i.totalAmount,isPay:!1,ratingID:""};I.push(c)}),g.patch(`https://teatimeapi-test.onrender.com/orders/${o}`,{orderDetail:I}).then(i=>{Le(v)}).catch(i=>{console.log(i)})}).catch(d=>{console.log(d)})}).catch(r=>{console.log(r)}),Pe(t)}function Le(t){g.get(`https://teatimeapi-test.onrender.com/groupings?UID=${t}&_expand=restaurant&_expand=order`).then(s=>{const e=s.data[0];let a=0;a=fe(e.order.orderDetail.length,e.restaurant.minGroupSize),$("#groupingProgressBar").html(`
      <div class="bar bg-brand-02" style="--value: ${a}">
        <div class="text-white">${a>100?100:a}%</div>
      </div>
    `)}).catch(s=>{console.log(s)})}function z(t,s,e){const a=document.getElementById("menuTotal");if(t<1e3&&t>=0){s.forEach((r,l)=>{r.orderItem==e&&(s[l].quantity=t)});let n=0;s.forEach(r=>{n+=r.totalAmount*r.quantity}),a.textContent=`$${n}`,localStorage.setItem("Carts",JSON.stringify(s))}else alert("點餐數量請介於0-999間")}function Pe(t){$("#orderTotal").html(`<p class="fw-md-bold fw-medium fs-20 fs-md-24 line-height-sm">$${t.orderPriceTotal}</p>`);const s=se(t.eventDateTime,t.deadlineDateTime);$("#eventDate").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="eventDate">${s.eventDateTime.date}</p>`),$("#eventDateTime").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="eventDateTime">${s.eventDateTime.time}</p>`),$("#deadlineDate").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="deadlineDate">${s.deadlineDateTime.date}</p>`),$("#deadlineTime").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="deadlineTime">${s.deadlineDateTime.time}</p>`),localStorage.setItem("Carts","")}function de(t,s,e,a=""){let n=0;a&&(n=fe(a.order.orderDetail.length,a.restaurant.minGroupSize));let r="";e?r+=`id=${e}`:r+=`UID=${s}`,$.ajax({url:`https://teatimeapi-test.onrender.com/restaurants?${r}`,success:function(l){let o=$("#paginationPagesMenu"),f=window.innerWidth<768;const{products:v,stars:d,category:u}=l[0];localStorage.setItem("category",u);let I=l.length,j=f?5:10,D="";const C=se(a.eventDateTime,a.deadlineDateTime);D+=`
                        ...
                        <div class="ms-auto ${t?"d-block":"d-none"}">
                          <ul>
                            <li class="d-flex align-items-center mb-16">
                              ...
                              /svg>
                              <p class="me-16 me-sm-8">${C.deadlineDateTime.date}</p>
                              <p class="">${C.deadlineDateTime.time}</p>
                            </li>
                            <li class="d-flex align-items-center mb-16">
                              ...
                              <p class='word-break-keep-all'>${a.invitees?a.invitees:"無"}</p>
                            </li>
                      ...
                      `,$("#storeOrder").html(D),o.pagination({dataSource:v,locator:"items",totalNumber:I,pageSize:j,showPageNumbers:!0,showPrevious:!0,showNext:!0,callback:function(w,T){const i=JSON.parse(JSON.stringify(w));let x="",c="",A="",re=i.slice(0,5),oe=i.slice(5),ye=[...oe];x+=`
                            <div class="swiper-slide border border-1 border-brand-03 p-12 p-sm-40">
                            <ul id="">
                          `,$.each(re,function(H,p){x+=`
                          ...
                          `,$.each(ye,function(H,p){c+=`
                            ...
                            </div>`,A+=x,j==5&&(c=""),A+=c,$(".customMenu").html(A),document.querySelectorAll(".menu-button").forEach(H=>{H.addEventListener("click",function(){let p="";const ve=this.getAttribute("data-menuuid");i.forEach(m=>{if(m.UID==ve){let R=function(){let b=parseInt(h.val())||0;b<0?(h.val(0),b=parseInt(h.val())||0):b>999&&(h.val(999),b=parseInt(h.val())||0);let y=b*S;$("#mealPrice").text(y)},M=!1;m.冷==!1&&m.熱==!1&&(M=!0),p+=`
                    ...
                    `,staticBackdrop.innerHTML=p;let $e=$("#numberSub"),h=$("#numberInput"),Ce=$("#numberAdd"),we=$("#addProductToCarts"),xe=m.價格,S=0;S=parseInt(xe),u=="飲料"&&$("[name='subscribe']").change(function(){$(this).is(":checked")?S+=10:S-=10,$("#mealPrice").text(`${S*h.val()}`)}),$e.on("click",function(){let b=parseInt(h.val())||0;b>1?(h.val(b-1),R()):h.val(1)}),Ce.on("click",function(){let b=parseInt(h.val())||0;b<999?(h.val(b+1),R()):h.val(999)}),h.on("input",R),we.on("click",function(){let b=[],y={};const B=[];if(y.orderItem=$("#modalOrderTitle").text(),y.quantity=$("#numberInput").val(),y.totalAmount=$("#numberInput").val()*S,u=="飲料"){y.ice=$('[name="temperature"]:checked').val(),y.sugar=$('[name="sweetness"]:checked').val();const U=$("#fresh_milk"),k=$("#pearl"),V=$("#brown_sugar"),Y=$("#agar");U.is(":checked")&&B.push("升級鮮奶"),k.is(":checked")&&B.push("珍珠"),V.is(":checked")&&B.push("黑糖"),Y.is(":checked")&&B.push("寒天晶球")}if(y.comments=$("#remark").val(),y.customization=B,y.originalPrice=m.價格,localStorage.getItem("Carts")){let U=0;const k=JSON.parse(localStorage.getItem("Carts"));k.forEach((V,Y)=>{V.orderItem===$("#modalOrderTitle").text()&&(U+=1,k[Y]=y)}),U==0&&k.push(y),localStorage.setItem("Carts",JSON.stringify(k))}else b.push(y),localStorage.setItem("Carts",JSON.stringify(b));$("#staticBackdrop").modal("hide")})}})})}),re=[],oe=[],new Swiper(".mySwiper",{breakpoints:{1:{slidesPerView:1,spaceBetween:10,centeredSlides:!1},576:{slidesPerView:2,spaceBetween:24,centeredSlides:!1}}})}})},error:function(l){console.error("error")}})}function ce(t,s,e){let a="";return t==!1&&s==!1?a+=`
    ...
    `,a}const Ge="https://teatimeapi-test.onrender.com",Oe=localStorage.getItem("token"),qe=ue(Oe);let G="",O="",Z=!1,ae="",J="";document.querySelector(".btnCreateGrouping");const be=document.getElementById("modal-CreateGroup"),Ne=document.getElementById("addGrouping"),me=document.getElementById("groupingInvitees");let K=location.href.split("=")[1];const He=!!K.startsWith("G");let q="";He?g.get(`https://teatimeapi-test.onrender.com/groupings?UID=${K}`).then(t=>{q=t.data[0].restaurantId}).catch(t=>{console.log(t)}):g.get(`https://teatimeapi-test.onrender.com/restaurants?UID=${K}`).then(t=>{q=t.data[0].id}).catch(t=>{console.log(t)});Ne.addEventListener("click",function(t){let s="";if(G==""){$(".EnjoyFillAlert").addClass("shake-element"),setTimeout(function(){$(".EnjoyFillAlert").removeClass("shake-element")},2e3);return}else{let e=new Date(G);const a=`${e.getMonth()+1}/${e.getDate()}`,n=`${e.getHours()}:${String(e.getMinutes()).padStart(2,"0")}`;$("#groupEnjoyDate").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="groupEnjoyDate">${a}</p>`),$("#groupEnjoyTime").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="groupEnjoyTime">${n}</p>`)}if(O==""){$(".orderFillAlert").addClass("shake-element"),setTimeout(function(){$(".orderFillAlert").removeClass("shake-element")},2e3);return}else{let e=new Date(O);const a=`${e.getMonth()+1}/${e.getDate()}`,n=`${e.getHours()}:${String(e.getMinutes()).padStart(2,"0")}`;$("#groupDeadlineDate").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="groupEnjoyDate">${a}</p>`),$("#groupDeadlineTime").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="groupEnjoyTime">${n}</p>`)}if(!Z){if(me.value==""){$(".inviteesFillAlert").addClass("shake-element"),setTimeout(function(){$(".inviteesFillAlert").removeClass("shake-element")},2e3);return}else s=me.value.toString()=="no-invitees"?"":`${ae}`;Re(s)}});function Re(t){let s={restaurantId:q,orderDetail:[]},e={restaurantId:q,deadlineDateTime:O.toString(),eventDateTime:G.toString(),initiatorId:J,initiator:ae.toString(),inviteesId:t?J:"",invitees:t?`${t}`:""};const a=g.get("https://teatimeapi-test.onrender.com/orders"),n=g.get("https://teatimeapi-test.onrender.com/groupings");g.all([a,n]).then(g.spread((r,l)=>{let o=r.data.length+1,f=l.data.length+1;s.groupingId=f,e.orderId=o;let v=Ve(o,f);s.UID=v.ordersUID,e.UID=v.groupingsUID,g.post("https://teatimeapi-test.onrender.com/orders",s).then(d=>g.post("https://teatimeapi-test.onrender.com/groupings",e).then(u=>{$("#modal-CreateGroup").modal("hide"),$("#modal-successfullyGroup").modal("show"),window.location.href=`store-order.html?UID=${e.UID}`}).catch(u=>{console.log(u)})).catch(d=>{console.log(d)})})).catch(r=>{console.log(r)})}function Ve(t,s){let e={},a=Math.floor(Math.log10(t))+1,n=Math.floor(Math.log10(s))+1;return a==1?e.ordersUID=`O0${t.toString().padStart(a+1,"0")}`:a==2?e.ordersUID=`O${t.toString().padStart(a+1,"0")}`:e.ordersUID=`O${t.toString().padStart(a,"0")}`,n==1?e.groupingsUID=`G0${s.toString().padStart(n+1,"0")}`:n==2?e.groupingsUID=`G${s.toString().padStart(n+1,"0")}`:e.groupingsUID=`G${s.toString().padStart(n,"0")}`,e}be.addEventListener("show.bs.modal",function(t){const s=document.querySelector(".groupingInitiator"),e=$("#storeNameID").html();$("#orderStoreId").html(`<p class="text-gray-01" id="orderStoreId">${e}</p>`),g.get(`${Ge}/users?UID=${qe}`).then(function(a){s.textContent=a.data[0].userName;const{userName:n,id:r}=a.data[0];ae=n,J=r}).catch(function(a){console.error(a.message)})});be.addEventListener("click",function(t){const s=document.getElementById("enjoymentDate"),e=document.getElementById("orderTerm"),a=document.getElementById("groupingInvitees"),n=s.value.trim(),r=e.value.trim();a.value.trim();let l="",o="";if(n&&(l=moment(n,"YYYY/MM/DD HH:mm")),r&&(o=moment(r,"YYYY/MM/DD HH:mm")),l&&o){const f=l.diff(o,"hours"),v=document.querySelector(".orderTermAlert");f<24?(v.classList.remove("d-none"),Z=!0):(v.classList.add("d-none"),Z=!1)}l&&$(".EnjoyFillAlert").addClass("d-none"),o&&$(".orderFillAlert").addClass("d-none"),$("#groupingInvitees").change(function(){var f=$(this).val();f===""?$(".inviteesFillAlert").show():$(".inviteesFillAlert").hide()})});ge("#enjoymentDate",{enableTime:!0,dateFormat:"Y/m/d H:i",time_24hr:!0,minuteIncrement:15,allowInput:!0,minDate:moment().format("YYYY/MM/DD"),onClose:function(t,s,e){G=s}});ge("#orderTerm",{enableTime:!0,dateFormat:"Y/m/d H:i",time_24hr:!0,minuteIncrement:5,allowInput:!0,minDate:moment().format("YYYY/MM/DD"),onClose:function(t,s,e){O=s}});const Ye="https://teatimeapi-test.onrender.com",Fe="/ratings",ze="_sort=id&_order=desc";let Q=document.querySelector(".rating-cards"),X=document.querySelector(".rating-cards-RWD"),ee=document.querySelector(".ratingToggleBtn"),te=document.querySelector(".ratingRWDToggleBtn"),N=[],he=[],E=window.location.search.slice(-4);(async function(){E.startsWith("G")&&(E=(await Promise.all([g.get(`https://teatimeapi-test.onrender.com/groupings?UID=${E}&_expand=restaurant`)]))[0].data[0].restaurant.UID),We()})();function We(){g.get(`${Ye}${Fe}?${ze}`).then(function(t){N=t.data,he=N.filter(s=>s.restaurantUID===E),ne()})}function ne(t=4){let s="";he.forEach((e,a)=>{if(e.restaurantUID===E&&a<t){let n=pe(e.starRating);s+=`
        ...
        </div>`}}),s!==""?(Q.innerHTML=s,X.innerHTML=s):(Q.innerHTML='<div class="d-flex justify-content-center fs-24 fw-bolder text-brand-03"> 此店家目前無評價</div>',ee.classList.add("d-none"),X.innerHTML='<div class="d-flex justify-content-center fs-16 fw-bolder text-brand-03"> 此店家目前無評價</div>',te.classList.add("d-none"))}te.addEventListener("click",function(){let t=!0;t=!t,X.innerHTML="",ne(t==!0?4:N.length),te.classList.toggle("transform180")});let W=!0;ee.addEventListener("click",function(){W=!W,Q.innerHTML="",ne(W==!0?4:N.length),ee.classList.toggle("transform180")});const Ze=document.getElementById("updateRatingBtn");Ze.addEventListener("click",Je);async function Je(){let t={},s=document.querySelector("[data-rating-star]").dataset.ratingStar,e=document.querySelector(".memberEvaluate");const a=document.querySelector("#memberModalLabel");e.value!==""&&(t={UID:"R004",reviewer:"黃莉琳",reviewerPhoto:"https://raw.githubusercontent.com/AnnChouCode/TeaTime-Gathering/main/assets/images/user/female/user-female-03.jpg",starRating:Number(s),feedbackText:e.value,reviewedRestaurant:a.textContent,restaurantUID:E,reviewDateTime:moment().format("YYYY/MM/DD")},g.post("https://teatimeapi-test.onrender.com/ratings",t).then(function(n){e.value="",document.querySelector("#modal-WriteReview .btn-close").click(),setTimeout(()=>{alert("留言評價成功 ,,・ω・,, "),location.reload()},100)}))}
