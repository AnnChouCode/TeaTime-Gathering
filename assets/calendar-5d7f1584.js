                      ...
                      <li class="d-flex gap-12 p-12 mb-12 border border-brand-03 border-radius-40401616">
                        ...
                        <div class="flex-grow-1 d-flex flex-column">
                          ...
                          <div class="d-flex justify-content-between flex-column flex-lg-row">
                            ...
                            <button
                              class="py-4 px-16 btn-brand-05 btn-hover-bg-1 border-0 rounded-pill text-brand-02 btnVoteThis ${o.currentVoters.includes(m)?"btn-active-brand-02":""}"
                              type="button" data-num=${o.id}>${o.currentVoters.includes(m)?"已投票":"投票"}</button>
                          </div>
                      </li>`}),t.innerHTML=n,Y()}function Y(){r.querySelectorAll(".btnVoteThis").forEach(t=>{t.addEventListener("click",function(){t.classList.contains("btn-active-brand-02")?(v(t.dataset.num,"deleteVote"),alert("投票已取消")):(v(t.dataset.num,"addVote"),alert("投票成功"))})})}function v(e,t){c.get(`${d}/votings/${e}`).then(function(n){const o=n.data.currentVoters;if(t==="addVote")o.push(`${m}`);else{const s=o.indexOf(`${m}`);o.splice(s,1)}c.patch(`${d}/votings/${e}`,{currentVoters:o}).then(function(s){y(s.data.UID)}).catch(function(s){console.error(s)})}).catch(function(n){console.error(n)})}const A=document.getElementById("modal-CreateVoting");A.addEventListener("click",()=>{const e=document.getElementById("eventDateTime"),t=document.getElementById("deadlineDateTime"),n=document.getElementById("votingInvitees"),o=e.value.trim(),s=t.value.trim();let i="",u="";o&&(i=moment(e.value,"YYYY/MM/DD HH:mm")),s&&(u=moment(t.value,"YYYY/MM/DD HH:mm")),i&&u&&(i.diff(u,"hours")<24?document.querySelector(".deadlineDateTimeAlert").classList.remove("d-none"):document.querySelector(".deadlineDateTimeAlert").classList.add("d-none")),sessionStorage.setItem("deadlineDateTime",s),sessionStorage.setItem("eventDateTime",o),sessionStorage.setItem("votingInvitees",n.value)});const k=document.querySelector(".btnLinkToCreateVoting");k.addEventListener("click",function(){window.location.href="./createVoting.html"});
