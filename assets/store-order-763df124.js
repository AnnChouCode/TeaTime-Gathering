import{b as P}from"./main-e0cad254.js";import{i as Ie,c as ue}from"./notification-17bffb4a.js";import{a as p}from"./enc-utf8-11b5c098.js";import{s as le}from"./showDateTime-51bda516.js";import{s as pe}from"./showStars-385adb56.js";import{i as De,c as ge}from"./calcGroupProgress-e070f845.js";import{f as fe}from"./index-a4e39586.js";const Te=localStorage.getItem("token");document.addEventListener("DOMContentLoaded",function(){const t=document.querySelector(".btnCreateGrouping"),a=document.querySelectorAll(".btnCart"),e=document.querySelectorAll(".btnWriteReview");Ie(Te)?(t.setAttribute("href","#modal-CreateGroup"),t.setAttribute("data-bs-toggle","modal"),e.forEach(s=>{s.setAttribute("href","#modal-WriteReview"),s.setAttribute("data-bs-toggle","modal")})):(t.setAttribute("data-bs-toggle","popover"),t.setAttribute("data-bs-content","請先登入帳號"),a.forEach(n=>{n.setAttribute("data-bs-toggle","popover"),n.setAttribute("data-bs-content","請先登入帳號")}),e.forEach(n=>{n.setAttribute("data-bs-toggle","popover"),n.setAttribute("data-bs-content","請先登入帳號")}),[...document.querySelectorAll('[data-bs-toggle="popover"]')].map(n=>{new P.Popover(n)}))});const Se=moment().format("YYYY/MM/DD"),ke="https://teatimeapi-test.onrender.com",F=location.href.split("=")[1],Ee=document.querySelector(".btnTogetherEvent");Ee.addEventListener("click",je);let _;function je(){const t=`_sort=deadlineDateTime&deadlineDateTime_gte=${Se}`;p.get(`${ke}/groupings?_expand=restaurant&${t}`).then(function(a){_=a.data;let e,s;F.startsWith("G")?(e=_.filter(r=>r.UID===F),s=e.length?e[0].restaurantId:""):(e=_.filter(r=>r.restaurant.UID===F),s=e.length?e[0].restaurantId:"");const n=_.filter(r=>r.restaurantId===s);Ae(n)}).catch(function(a){console.error(a.message)})}function Ae(t){const a=document.querySelector(".calendarBlock");let e="";t.length?t.forEach(s=>{e+=`<div class="col-md-6 col-xl-4 col-xxxl-3 px-0 px-md-12">
                    ...
                    </div>`}):e="<p class='fs-16 fs-md-20 text-center'>目前沒有開團</p>",a.innerHTML=e}function se(t="",a=""){let e={eventDateTime:{},deadlineDateTime:{}};if(t!=""){const[s,n]=t.split(" "),l=new Date(t).toLocaleDateString("en-US",{month:"numeric",day:"numeric"});e.eventDateTime.yymmdd=s,e.eventDateTime.date=l,e.eventDateTime.time=n}if(a!=""){const[s,n]=a.split(" "),l=new Date(a).toLocaleDateString("en-US",{month:"numeric",day:"numeric"});e.deadlineDateTime.yymmdd=s,e.deadlineDateTime.date=l,e.deadlineDateTime.time=n}return e}let Me=localStorage.getItem("token");const L=ue(Me);localStorage.setItem("Carts","");localStorage.setItem("category","");const ie=document.querySelector(".shopping-cart");document.querySelector(".shopping-cart-mobile ");const Be=document.getElementById("modalCartsSendOrder");$(function(){(function(){new P.Modal(document.getElementById("modal-shppingCart-order")),document.getElementById("staticBackdrop");let t=location.href.split("=")[1],a="";const e=!!t.startsWith("G");e?p.get(`https://teatimeapi-test.onrender.com/groupings?UID=${t}&_expand=restaurant&_expand=order`).then(s=>{if(!s.data[0]){$("#modal-storeOrder-eventExpired").modal("show"),$("#storeOrderClose").on("click",function(){window.location.href="https://annchoucode.github.io/TeaTime-Gathering"});return}const{restaurantId:n,deadlineDateTime:r}=s.data[0],l=De(r);l&&new P.Popover(ie,{content:"揪團活動已截止"}),a=n,de(e,t,a,s.data[0]),Ue(s.data[0],l)}).catch(s=>{console.log(s)}):(new P.Popover(ie,{content:"目前您未選擇揪團活動，請先選擇揪團中或建立揪團"}),de(e,t,a,""))})()});function Ue(t,a){$(".shopping-cart").on("click",function(){if(L&&!a)$("#modal-shppingCart-order").modal("show");else return;localStorage.getItem("category");let e="";t.storeName=$("#storeNameID").text();const s=se(t.eventDateTime,t.deadlineDateTime);t.yymmdd=s.eventDateTime.yymmdd,t.date=s.eventDateTime.date,t.time=s.eventDateTime.time,$("#modalShppingCartOrderTitle").html(`<p class="fs-20 fs-md-24 line-height-sm fw-bold text-gray-01" id="modalShppingCartOrderTitle">${t.storeName}</p>`),$("#dateCarts").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="dateCarts">${s.eventDateTime.date}</p>`),$("#timeCarts").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="timeCarts">${s.eventDateTime.time}</p>`),$("#dateCartsOrder").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="dateCartsOrder">${s.deadlineDateTime.date}</p>`),$("#timeCartsOrder").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="timeCartsOrder">${s.deadlineDateTime.time}</p>`);const n=localStorage.getItem("Carts");if(n.length!==0){$(".modalCartsFooter").show();const r=JSON.parse(n);let l=0;r.forEach(o=>{l+=parseInt(o.totalAmount);let g="";o.customization.length!==0&&(g=o.customization.join(", ")),e+=`
