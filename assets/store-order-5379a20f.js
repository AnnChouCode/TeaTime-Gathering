import{b as L}from"./main-96991ec2.js";import{i as Ie,c as ue}from"./notification-39e0ad74.js";import{a as p}from"./enc-utf8-c5181946.js";import{s as le}from"./showDateTime-51bda516.js";import{s as pe}from"./showStars-385adb56.js";import{f as ge}from"./index-a4e39586.js";const De=localStorage.getItem("token");document.addEventListener("DOMContentLoaded",function(){const e=document.querySelector(".btnCreateGrouping"),a=document.querySelectorAll(".btnCart"),t=document.querySelectorAll(".btnWriteReview");Ie(De)?(e.setAttribute("href","#modal-CreateGroup"),e.setAttribute("data-bs-toggle","modal"),a.forEach(s=>{}),t.forEach(s=>{s.setAttribute("href","#modal-WriteReview"),s.setAttribute("data-bs-toggle","modal")})):(e.setAttribute("data-bs-toggle","popover"),e.setAttribute("data-bs-content","請先登入帳號"),a.forEach(n=>{n.setAttribute("data-bs-toggle","popover"),n.setAttribute("data-bs-content","請先登入帳號")}),t.forEach(n=>{n.setAttribute("data-bs-toggle","popover"),n.setAttribute("data-bs-content","請先登入帳號")}),[...document.querySelectorAll('[data-bs-toggle="popover"]')].map(n=>{new L.Popover(n)}))});const Te=moment().format("YYYY/MM/DD"),Se="https://teatimeapi-test.onrender.com",F=location.href.split("=")[1],ke=document.querySelector(".btnTogetherEvent");ke.addEventListener("click",Ee);let _;function Ee(){const e=`_sort=deadlineDateTime&deadlineDateTime_gte=${Te}`;p.get(`${Se}/groupings?_expand=restaurant&${e}`).then(function(a){_=a.data;let t,s;F.startsWith("G")?(t=_.filter(r=>r.UID===F),s=t.length?t[0].restaurantId:""):(t=_.filter(r=>r.restaurant.UID===F),s=t.length?t[0].restaurantId:"");const n=_.filter(r=>r.restaurantId===s);je(n)}).catch(function(a){console.error(a.message)})}function je(e){const a=document.querySelector(".calendarBlock");let t="";e.length?e.forEach(s=>{t+=`<div class="col-md-6 col-xl-4 col-xxxl-3 px-0 px-md-12">
                    ...
                    </div>`}):t="<p class='fs-16 fs-md-20 text-center'>目前沒有開團</p>",a.innerHTML=t}function se(e="",a=""){let t={eventDateTime:{},deadlineDateTime:{}};if(e!=""){const[s,n]=e.split(" "),l=new Date(e).toLocaleDateString("en-US",{month:"numeric",day:"numeric"});t.eventDateTime.yymmdd=s,t.eventDateTime.date=l,t.eventDateTime.time=n}if(a!=""){const[s,n]=a.split(" "),l=new Date(a).toLocaleDateString("en-US",{month:"numeric",day:"numeric"});t.deadlineDateTime.yymmdd=s,t.deadlineDateTime.date=l,t.deadlineDateTime.time=n}return t}function fe(e=0,a=0){return Math.floor(e/a*100)}function Ae(e){const a=new Date().getTime(),t=Date.parse(e);return a>t}let Me=localStorage.getItem("token");const P=ue(Me);localStorage.setItem("Carts","");localStorage.setItem("category","");const ie=document.querySelector(".shopping-cart");document.querySelector(".shopping-cart-mobile ");const Be=document.getElementById("modalCartsSendOrder");$(function(){(function(){new L.Modal(document.getElementById("modal-shppingCart-order")),document.getElementById("staticBackdrop");let e=location.href.split("=")[1],a="";const t=!!e.startsWith("G");t?p.get(`https://teatimeapi-test.onrender.com/groupings?UID=${e}&_expand=restaurant&_expand=order`).then(s=>{if(!s.data[0]){$("#modal-storeOrder-eventExpired").modal("show"),$("#storeOrderClose").on("click",function(){window.location.href="https://annchoucode.github.io/TeaTime-Gathering"});return}const{restaurantId:n,deadlineDateTime:r}=s.data[0],l=Ae(r);l&&new L.Popover(ie,{content:"揪團活動已截止"}),a=n,de(t,e,a,s.data[0]),Ue(s.data[0],l)}).catch(s=>{console.log(s)}):(new L.Popover(ie,{content:"目前您未選擇揪團活動，請先選擇揪團中或建立揪團"}),de(t,e,a,""))})()});function Ue(e,a){$(".shopping-cart").on("click",function(){if(P&&!a)$("#modal-shppingCart-order").modal("show");else return;localStorage.getItem("category");let t="";e.storeName=$("#storeNameID").text();const s=se(e.eventDateTime,e.deadlineDateTime);e.yymmdd=s.eventDateTime.yymmdd,e.date=s.eventDateTime.date,e.time=s.eventDateTime.time,$("#modalShppingCartOrderTitle").html(`<p class="fs-20 fs-md-24 line-height-sm fw-bold text-gray-01" id="modalShppingCartOrderTitle">${e.storeName}</p>`),$("#dateCarts").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="dateCarts">${s.eventDateTime.date}</p>`),$("#timeCarts").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="timeCarts">${s.eventDateTime.time}</p>`),$("#dateCartsOrder").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="dateCartsOrder">${s.deadlineDateTime.date}</p>`),$("#timeCartsOrder").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="timeCartsOrder">${s.deadlineDateTime.time}</p>`);const n=localStorage.getItem("Carts");if(n.length!==0){$(".modalCartsFooter").show();const r=JSON.parse(n);let l=0;r.forEach(o=>{l+=parseInt(o.totalAmount);let g="";o.customization.length!==0&&(g=o.customization.join(", ")),t+=`
      ...
      `,$("#menuListUl").html(t),$(".modalCartsFooter").hide()}),Be.addEventListener("click",function(){_e(e)})}function _e(e){if(!P)return;$("#modal-shppingCart-order").modal("hide"),$("#modal-successfullyOrdered").modal("show");const a=document.getElementById("menuTotal"),t=parseInt(a.textContent.replace("$","")),s=JSON.parse(localStorage.getItem("Carts")),n=JSON.parse(JSON.stringify(s));e.orderPriceTotal=t,e.orderUserId=P,p.get(`https://teatimeapi-test.onrender.com/users?UID=${P}`).then(r=>{const{userName:l}=r.data[0],{orderId:b,UID:o}=e,g=o;return e.userName=l,p.get(`https://teatimeapi-test.onrender.com/orders?id=${b}`).then(w=>{const i=w.data[0],{orderDetail:x,groupingId:T,restaurantId:j,UID:I}=i,h=[];let D="";return x.forEach((d,C)=>{s.forEach((c,A)=>{c.customization.length!=0&&(D=c.customization.join("、")),d.name==l&&d.orderItem==c.orderItem&&(h.push(A),x[C]={name:l,orderItem:c.orderItem,quantity:parseInt(c.quantity),ice:c.ice?`${c.ice}`:"",sugar:c.sugar?`${c.sugar}`:"",customization:c.customization?`${D}`:"",comments:c.comments,totalAmount:c.totalAmount,isPay:d.isPay,ratingID:d.ratingID})})}),h.length>0&&h.reverse().forEach(C=>{n.splice(C,1)}),n.forEach(d=>{let C="";d.customization.length!=0&&(C=d.customization.join("、"));const c={name:l,orderItem:d.orderItem,quantity:parseInt(d.quantity),ice:d.ice?`${d.ice}`:"",sugar:d.sugar?`${d.sugar}`:"",customization:d.customization?`${C}`:"",comments:d.comments,totalAmount:d.totalAmount,isPay:!1,ratingID:""};x.push(c)}),p.patch(`https://teatimeapi-test.onrender.com/orders/${b}`,{orderDetail:x}).then(d=>{Le(g)}).catch(d=>{console.log(d)})}).catch(w=>{console.log(w)})}).catch(r=>{console.log(r)}),Pe(e)}function Le(e){p.get(`https://teatimeapi-test.onrender.com/groupings?UID=${e}&_expand=restaurant&_expand=order`).then(a=>{const t=a.data[0];let s=0;s=fe(t.order.orderDetail.length,t.restaurant.minGroupSize),$("#groupingProgressBar").html(`
      <div class="bar bg-brand-02" style="--value: ${s}">
        <div class="text-white">${s>100?100:s}%</div>
      </div>
    `)}).catch(a=>{console.log(a)})}function z(e,a,t){const s=document.getElementById("menuTotal");if(e<1e3&&e>=0){a.forEach((r,l)=>{r.orderItem==t&&(a[l].quantity=e)});let n=0;a.forEach(r=>{n+=r.totalAmount*r.quantity}),s.textContent=`$${n}`,localStorage.setItem("Carts",JSON.stringify(a))}else alert("點餐數量請介於0-999間")}function Pe(e){$("#orderTotal").html(`<p class="fw-md-bold fw-medium fs-20 fs-md-24 line-height-sm">$${e.orderPriceTotal}</p>`);const a=se(e.eventDateTime,e.deadlineDateTime);$("#eventDate").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="eventDate">${a.eventDateTime.date}</p>`),$("#eventDateTime").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="eventDateTime">${a.eventDateTime.time}</p>`),$("#deadlineDate").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="deadlineDate">${a.deadlineDateTime.date}</p>`),$("#deadlineTime").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="deadlineTime">${a.deadlineDateTime.time}</p>`),localStorage.setItem("Carts","")}function de(e,a,t,s=""){let n=0;s&&(n=fe(s.order.orderDetail.length,s.restaurant.minGroupSize));let r="";t?r+=`id=${t}`:r+=`UID=${a}`,$.ajax({url:`https://teatimeapi-test.onrender.com/restaurants?${r}`,success:function(l){let b=$("#paginationPagesMenu"),o=window.innerWidth<768;const{products:g,stars:w,category:i}=l[0];localStorage.setItem("category",i);let x=l.length,T=o?5:10,j="";const I=se(s.eventDateTime,s.deadlineDateTime);j+=`
    ...
    `,s}const Ge="https://teatimeapi-test.onrender.com",Oe=localStorage.getItem("token"),qe=ue(Oe);let G="",O="",Z=!1,ae="",J="";document.querySelector(".btnCreateGrouping");const be=document.getElementById("modal-CreateGroup"),Ne=document.getElementById("addGrouping"),me=document.getElementById("groupingInvitees");let K=location.href.split("=")[1];const He=!!K.startsWith("G");let q="";He?p.get(`https://teatimeapi-test.onrender.com/groupings?UID=${K}`).then(e=>{q=e.data[0].restaurantId}).catch(e=>{console.log(e)}):p.get(`https://teatimeapi-test.onrender.com/restaurants?UID=${K}`).then(e=>{q=e.data[0].id}).catch(e=>{console.log(e)});Ne.addEventListener("click",function(e){let a="";if(G==""){$(".EnjoyFillAlert").addClass("shake-element"),setTimeout(function(){$(".EnjoyFillAlert").removeClass("shake-element")},2e3);return}else{let t=new Date(G);const s=`${t.getMonth()+1}/${t.getDate()}`,n=`${t.getHours()}:${String(t.getMinutes()).padStart(2,"0")}`;$("#groupEnjoyDate").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="groupEnjoyDate">${s}</p>`),$("#groupEnjoyTime").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="groupEnjoyTime">${n}</p>`)}if(O==""){$(".orderFillAlert").addClass("shake-element"),setTimeout(function(){$(".orderFillAlert").removeClass("shake-element")},2e3);return}else{let t=new Date(O);const s=`${t.getMonth()+1}/${t.getDate()}`,n=`${t.getHours()}:${String(t.getMinutes()).padStart(2,"0")}`;$("#groupDeadlineDate").html(`<p class="me-8 fs-16 fs-md-20 fw-medium line-height-sm" id="groupEnjoyDate">${s}</p>`),$("#groupDeadlineTime").html(`<p class="fs-16 fs-md-20 fw-medium line-height-sm" id="groupEnjoyTime">${n}</p>`)}if(!Z){if(me.value==""){$(".inviteesFillAlert").addClass("shake-element"),setTimeout(function(){$(".inviteesFillAlert").removeClass("shake-element")},2e3);return}else a=me.value.toString()=="no-invitees"?"":`${ae}`;Re(a)}});function Re(e){let a={restaurantId:q,orderDetail:[]},t={restaurantId:q,deadlineDateTime:O.toString(),eventDateTime:G.toString(),initiatorId:J,initiator:ae.toString(),inviteesId:e?J:"",invitees:e?`${e}`:""};const s=p.get("https://teatimeapi-test.onrender.com/orders"),n=p.get("https://teatimeapi-test.onrender.com/groupings");p.all([s,n]).then(p.spread((r,l)=>{let b=r.data.length+1,o=l.data.length+1;a.groupingId=o,t.orderId=b;let g=Ve(b,o);a.UID=g.ordersUID,t.UID=g.groupingsUID,p.post("https://teatimeapi-test.onrender.com/orders",a).then(w=>p.post("https://teatimeapi-test.onrender.com/groupings",t).then(i=>{$("#modal-CreateGroup").modal("hide"),$("#modal-successfullyGroup").modal("show"),window.location.href=`store-order.html?UID=${t.UID}`}).catch(i=>{console.log(i)})).catch(w=>{console.log(w)})})).catch(r=>{console.log(r)})}function Ve(e,a){let t={},s=Math.floor(Math.log10(e))+1,n=Math.floor(Math.log10(a))+1;return s==1?t.ordersUID=`O0${e.toString().padStart(s+1,"0")}`:s==2?t.ordersUID=`O${e.toString().padStart(s+1,"0")}`:t.ordersUID=`O${e.toString().padStart(s,"0")}`,n==1?t.groupingsUID=`G0${a.toString().padStart(n+1,"0")}`:n==2?t.groupingsUID=`G${a.toString().padStart(n+1,"0")}`:t.groupingsUID=`G${a.toString().padStart(n,"0")}`,t}be.addEventListener("show.bs.modal",function(e){const a=document.querySelector(".groupingInitiator"),t=$("#storeNameID").html();$("#orderStoreId").html(`<p class="text-gray-01" id="orderStoreId">${t}</p>`),p.get(`${Ge}/users?UID=${qe}`).then(function(s){a.textContent=s.data[0].userName;const{userName:n,id:r}=s.data[0];ae=n,J=r}).catch(function(s){console.error(s.message)})});be.addEventListener("click",function(e){const a=document.getElementById("enjoymentDate"),t=document.getElementById("orderTerm"),s=document.getElementById("groupingInvitees"),n=a.value.trim(),r=t.value.trim();s.value.trim();let l="",b="";if(n&&(l=moment(n,"YYYY/MM/DD HH:mm")),r&&(b=moment(r,"YYYY/MM/DD HH:mm")),l&&b){const o=l.diff(b,"hours"),g=document.querySelector(".orderTermAlert");o<24?(g.classList.remove("d-none"),Z=!0):(g.classList.add("d-none"),Z=!1)}l&&$(".EnjoyFillAlert").addClass("d-none"),b&&$(".orderFillAlert").addClass("d-none"),$("#groupingInvitees").change(function(){var o=$(this).val();o===""?$(".inviteesFillAlert").show():$(".inviteesFillAlert").hide()})});ge("#enjoymentDate",{enableTime:!0,dateFormat:"Y/m/d H:i",time_24hr:!0,minuteIncrement:15,allowInput:!0,minDate:moment().format("YYYY/MM/DD"),onClose:function(e,a,t){G=a}});ge("#orderTerm",{enableTime:!0,dateFormat:"Y/m/d H:i",time_24hr:!0,minuteIncrement:5,allowInput:!0,minDate:moment().format("YYYY/MM/DD"),onClose:function(e,a,t){O=a}});const Ye="https://teatimeapi-test.onrender.com",Fe="/ratings",ze="_sort=id&_order=desc";let Q=document.querySelector(".rating-cards"),X=document.querySelector(".rating-cards-RWD"),ee=document.querySelector(".ratingToggleBtn"),te=document.querySelector(".ratingRWDToggleBtn"),N=[],he=[],E=window.location.search.slice(-4);(async function(){E.startsWith("G")&&(E=(await Promise.all([p.get(`https://teatimeapi-test.onrender.com/groupings?UID=${E}&_expand=restaurant`)]))[0].data[0].restaurant.UID),We()})();function We(){p.get(`${Ye}${Fe}?${ze}`).then(function(e){N=e.data,he=N.filter(a=>a.restaurantUID===E),ne()})}function ne(e=4){let a="";he.forEach((t,s)=>{if(t.restaurantUID===E&&s<e){let n=pe(t.starRating);a+=`
        <div class="feedback-style col-12 col-md-6 pt-32 pt-sm-48 mb-md-32">
            <div class="bg-brand-05 border-radius-16 px-12 px-sm-24 pb-12 pb-sm-24 custom-shadow-025">
                ...
                <img class="people-img border-radius-24 border border-2 border-line me-16" src=${t.reviewerPhoto} alt="user-img">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <p class="fs-16 fs-md-20 text-gray-02">${t.reviewer}</p>
                        <time datetime="2023-09-01" class="">${t.reviewDateTime}</time>
                    </div>
                    </div>
                </div>
                <p class="line-clamp-2">${t.feedbackText}</p>
            </div>
        </div>`}}),a!==""?(Q.innerHTML=a,X.innerHTML=a):(Q.innerHTML='<div class="d-flex justify-content-center fs-24 fw-bolder text-brand-03"> 此店家目前無評價</div>',ee.classList.add("d-none"),X.innerHTML='<div class="d-flex justify-content-center fs-16 fw-bolder text-brand-03"> 此店家目前無評價</div>',te.classList.add("d-none"))}te.addEventListener("click",function(){let e=!0;e=!e,X.innerHTML="",ne(e==!0?4:N.length),te.classList.toggle("transform180")});let W=!0;ee.addEventListener("click",function(){W=!W,Q.innerHTML="",ne(W==!0?4:N.length),ee.classList.toggle("transform180")});const Ze=document.getElementById("updateRatingBtn");Ze.addEventListener("click",Je);async function Je(){let e={},a=document.querySelector("[data-rating-star]").dataset.ratingStar,t=document.querySelector(".memberEvaluate");const s=document.querySelector("#memberModalLabel");t.value!==""&&(e={UID:"R004",reviewer:"黃莉琳",reviewerPhoto:"https://raw.githubusercontent.com/AnnChouCode/TeaTime-Gathering/main/assets/images/user/female/user-female-03.jpg",starRating:Number(a),feedbackText:t.value,reviewedRestaurant:s.textContent,restaurantUID:E,reviewDateTime:moment().format("YYYY/MM/DD")},p.post("https://teatimeapi-test.onrender.com/ratings",e).then(function(n){t.value="",document.querySelector("#modal-WriteReview .btn-close").click(),setTimeout(()=>{alert("留言評價成功 ,,・ω・,, "),location.reload()},100)}))}
